...
(functions and methods), package-level consts and vars, and defining files.
Check it before reading source: navigation questions
...
.item Never edit pkg.fact. It is a generated, read-only lens over the Go
source. To change a fact, edit the source it reflects.
.item With the hooks wired (below), regeneration is automatic: the
...
.item Manual fallback when hooks are missing or report errors: regenerate
with fact project -w <pkg-dir> and commit the result with the source
change.
.item Read the pkg.fact diff as the impact report of your edit: renamed
facts, changed signatures, and updated caller lists are exactly the blast
radius — no noise.
.item If pkg.fact and the source seem to disagree, the projection is stale:
...
.item 3. CI: fact project -check <pkg-dir> for each projected package —
