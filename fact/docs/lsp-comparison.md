...
Design rationale, not normative material — the spec (SPEC.md) defines the
...
- It writes whole files to disk, so the server learns of changes via
  file-watcher events (extra latency, debouncing) rather than in-band
  buffer edits.
- It edits several files back-to-back, then immediately asks a global
  question.
- The protocol has no barrier primitive — no standard request meaning
...
- pkg.fact answers the **declaration layer only**. Position-precise
...
- On very large modules, per-edit regeneration of a heavily-depended-on
  package costs more than 0.2s — though still per-package, never
  workspace-wide.
- During read-only exploration (no edits in flight), LSP staleness does
